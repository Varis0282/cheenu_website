        ...
        <div className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
          {groupRegions.map((r, i) => (
            <Reveal key={r.name} delay={i * 0.07} className="h-full">
              <Link
                href="/group-tours"
                className="card-glow group relative block h-44 overflow-hidden rounded-2xl border border-white/10"
              >
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[1.5fr_1fr]">
            ...
            <div className="h-full rounded-3xl border border-white/10 bg-white/[0.03] p-7">
              ...
              <ul className="mt-5 grid gap-x-6 gap-y-3 sm:grid-cols-2">
                {tourInclusions.map((inc) => (
                  <li key={inc} className="flex items-start gap-2.5 text-sm text-white/75">
          ...
          <Reveal delay={0.1} className="h-full">
            <div className="flex h-full flex-col justify-between rounded-3xl border border-gold/25 bg-gradient-to-br from-gold/15 to-transparent p-7">
              <div>
                <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-gold/20 text-gold">
